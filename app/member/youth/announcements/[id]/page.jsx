...
import { getYouthAnnouncementById } from "@/lib/content";
import { formatMemberDate } from "@/lib/member-page-data";
import { IconCalendarWeek } from "@tabler/icons-react";

export default async function YouthAnnouncementDetailPage({ params }) {
  const resolvedParams = await params;
  const announcement = await getYouthAnnouncementById(resolvedParams?.id);
  ...
  return (
    <AppShell navKey="youth" theme="youth" title="Youth Announcement Detail" subtitle="Full youth announcement reading page.">
      <BackRow fallbackHref="/member/youth/announcements" useHistory={false} />
      ...
      {announcement ? (
        ...
        <section className="lc-card alt">
          <p className="lc-muted">That youth announcement is no longer available.</p>
        </section>
      )}
    </AppShell>
  );
