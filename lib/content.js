...
const FALLBACK_YOUTH_ANNOUNCEMENTS = [
  {
    id: "fallback-youth",
    title: "Youth Update",
    body: "Check back soon for upcoming youth events.",
    imageUrl: null,
    imageAlt: null,
  },
];
...
const FALLBACK_YOUTH_SCRIPTURE = {
  ...
  devotional_text: "Take this verse with you this week and ask God how to live it out today.",
};
...
const FALLBACK_MEMBER_SCRIPTURE = {
  ...
  devotional_text: "Let this verse shape the way you walk with Christ this week.",
};
...
function normalizeAnnouncements(items) {
  ...
  return items.map((item) => ({
    ...
    imageAlt: String(item?.image_alt || "").trim() || null,
...
function normalizeSocialLink(item) {
  const platformKey = String(item?.platform_key || "").trim().toLowerCase();
  const label = String(item?.label || "").trim();
  const url = String(item?.url || "").trim();

  if (!platformKey || !label || !url) {
    return null;
  }
  ...
  return {
    ...
    label,
    url,
    sortOrder: Number.parseInt(String(item?.sort_order ?? 0), 10) || 0,
...
async function getAnnouncementsByCategory(supabase, category, limit = 8) {
  ...
  const { data, error } = await supabase
    .from("announcements")
    .select("id, title, body, image_url, image_alt, starts_at, created_at")
...
async function getHomepageMinistries(supabase, limit = 12) {
  ...
  const { data, error } = await supabase
    .from("ministries")
    .select("id, title, body, image_url, image_alt")
    .eq("is_published", true)
    .order("sort_order", { ascending: true })
...
async function getSocialLinks(supabase) {
  if (!supabase) {
    return [];
  }
  ...
  const { data, error } = await supabase
    .from("social_links")
    .select("id, platform_key, label, url, sort_order, is_active, created_at, updated_at")
    .eq("is_active", true)
    .order("sort_order", { ascending: true })
  ...
  return data.map(normalizeSocialLink).filter(Boolean);
}
...
async function getScriptureByAudience(supabase, audience) {
  ...
  const withAudience = await supabase
    .from("scriptures")
    .select("title, reference, verse_text, devotional_text")
...
async function getLatestLivestreamRecord(supabase, { activeOnly = false } = {}) {
  if (!supabase) {
    return null;
  }
  ...
  let query = supabase
    .from("livestreams")
    .select("title, embed_url, fallback_video_url, is_active")
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
...
export async function getLivestreamContent() {
  ...
  const liveYoutubeApiKey = readFirstEnv(
    "YOUTUBE_LIVE_API_KEY",
    "SUPABASE_NEW_LIVE_KEY",
    "YOUTUBE_API_KEY",
  );
  const liveYoutubeChannelId = readFirstEnv(
    "YOUTUBE_LIVE_CHANNEL_ID",
    "YOUTUBE_CHANNEL_ID",
  );
  const canCheckYouTubeLive = Boolean(liveYoutubeApiKey && liveYoutubeChannelId);

  const [latestStream, activeStream, youtubeLive] = await Promise.all([
    getLatestLivestreamRecord(supabase),
    getLatestLivestreamRecord(supabase, { activeOnly: true }),
    canCheckYouTubeLive
      ? getCurrentLiveVideo({
        apiKey: liveYoutubeApiKey,
        channelId: liveYoutubeChannelId,
      })
  ...
  if (activeStream?.embed_url) {
    return {
      ...
      isLive: true,
      liveEmbedUrl: activeStream.embed_url,
      fallbackVideoUrl:
        activeStream.fallback_video_url ||
        latestStream?.fallback_video_url ||
        fallbackVideoUrl,
      note: fallbackNote,
    };
  }

  if (!latestStream && youtubeLive?.embedUrl) {
    return {
      source: "youtube",
  ...
  return {
    ...
    isLive: false,
    liveEmbedUrl: "",
    fallbackVideoUrl: latestStream?.fallback_video_url || fallbackVideoUrl,
    note: fallbackNote,
  };
}
...
export async function getSocialLinksContent() {
  const supabase = await createSupabaseServerClient();
  return getSocialLinks(supabase);
}
...
export async function getYouthAnnouncementsContent(limit = 24) {
  const supabase = await createSupabaseServerClient();
  return await getAnnouncementsByCategory(supabase, "youth", limit);
}
...
export async function getYouthAnnouncementById(announcementId) {
  const supabase = await createSupabaseServerClient();
  if (!supabase || !announcementId) {
    return null;
  }
...
export async function getSermonsContent() {
  const supabase = await createSupabaseServerClient();
  const sermonsYoutubeApiKey = readFirstEnv(
    "YOUTUBE_SERMONS_API_KEY",
    "SUPABASE_NEW_SERMONS_KEY",
    "YOUTUBE_API_KEY",
  );
  const sermonsYoutubeChannelId = readFirstEnv(
    "YOUTUBE_SERMONS_CHANNEL_ID",
    "YOUTUBE_CHANNEL_ID",
  );

  const youtubeVideos = sermonsYoutubeApiKey && sermonsYoutubeChannelId
    ? await getChannelRecentVideos({
      apiKey: sermonsYoutubeApiKey,
      channelId: sermonsYoutubeChannelId,
      maxResults: 24,
    })
    : [];
