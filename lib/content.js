...
const DEFAULT_FALLBACK_STREAM_VIDEO =
...
const FALLBACK_MINISTRIES = [
  {
    id: "fallback-ministry-1",
    title: "Men's Fellowship",
    body: "First Thursday each month, 6:00 PM. Come hang out, build friendships, dinner and fellowship.",
  },
  {
    id: "fallback-ministry-2",
    title: "Children's Church",
    body: "Is offered during the sermon each Sunday, except for the last Sunday of the month.",
  },
  {
    id: "fallback-ministry-3",
    title: "Join a Serve Team",
    body: "Media, worship, greeters, and kids teams are growing. Ask at the Info Table.",
  },
  {
    id: "fallback-ministry-4",
    title: "Nursery Available!",
    body: "We'd love to care for your little ones! Our nursery is open during every service, offering a safe and loving place for them while you worship.",
  },
];
...
const FALLBACK_MAIN_SCRIPTURE = {
  reference: "John 3:16",
  verse_text:
    "For God so loved the world that He gave His one and only Son, that whoever believes in Him shall not perish but have eternal life.",
};

const FALLBACK_SEASONAL_FEATURE = {
  id: "fallback-seasonal",
  title: "This Season at Liberty Church",
  body: "Join us as we grow together in faith, fellowship, and service through every season.",
  scripture_reference: "Lamentations 3:22-23",
  scripture_text: "His mercies are new every morning; great is Your faithfulness.",
  media_url: "",
  media_type: "",
  cta_label: "",
  cta_url: "",
...
async function getHomepageSeasonalFeature(supabase) {
  ...
  const now = Date.now();
  const activeByWindow = data.find((item) => {
...
export async function getHomepageContent() {
  ...
  const [announcements, ministries, seasonalFeature, scripture, livestream] = await Promise.all([
    getAnnouncementsByCategory(supabase, "main", 8),
    getHomepageMinistries(supabase, 12),
    getHomepageSeasonalFeature(supabase),
    getScriptureByAudience(supabase, "main"),
  ...
  return {
    ...
    seasonalFeature: seasonalFeature || FALLBACK_SEASONAL_FEATURE,
    scripture: scripture || FALLBACK_MAIN_SCRIPTURE,
    livestream,
  };
}
