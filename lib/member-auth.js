...
async function getMemberByAuthUserId(supabase, authUserId) {
  ...
  const { data, error } = await supabase
    .from("team_members")
    .select("id, username, full_name, email, phone, auth_user_id, is_active, is_superuser, notes, created_at, last_login_at")
...
export function getMissingMemberAuthConfig() {
  ...
  if (!hasSupabaseAdminRuntimeConfig()) missing.push("SUPABASE_SERVICE_ROLE_KEY");
  return missing;
}
