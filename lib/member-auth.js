...
import { createSupabaseAdminClient, getSupabaseServerSecretKey } from "@/lib/supabase/admin";
...
export function getMemberProfilePhotoUrl(userLike) {
  ...
  const candidates = [
    metadata.profile_photo_url, metadata.profilePhotoUrl, metadata.photo_url, metadata.avatar_url, metadata.picture,
    userLike?.profile_photo_url, userLike?.profilePhotoUrl, userLike?.photo_url, userLike?.photoUrl,
  ];
...
export function normalizeMemberUsername(value) {
  return String(value || "").trim().toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/-+/g, "-").replace(/^[-._]+|[-._]+$/g, "").slice(0, 48);
}
...
async function getMemberByAuthUserId(supabase, authUserId) {
  ...
  const { data, error } = await supabase.from("team_members").select("id, username, full_name, email, phone, auth_user_id, is_active, is_superuser, notes, created_at, last_login_at").eq("auth_user_id", authUserId).maybeSingle();
  return error ? null : data || null;
}

async function getMemberByEmail(supabase, email) {
  if (!email) return null;
  const { data, error } = await supabase.from("team_members").select("id, username, full_name, email, phone, auth_user_id, is_active, is_superuser, notes, created_at, last_login_at").eq("email", email).maybeSingle();
  return error ? null : data || null;
}
...
export function getMissingMemberAuthConfig() {
  ...
  if (!getSupabaseServerSecretKey()) missing.push("SUPABASE_SECRET_KEY");
  return missing;
}
...
export async function ensureMemberProfileForAuthUser({ user, fullName = null, phone = null, notes = null, updateLastLoginAt = false }) {
  if (!isMemberAuthConfigured() || !user?.id) return null;
  const supabase = createSupabaseAdminClient();
  if (!supabase) return null;

  const email = normalizeEmail(user.email);
  const resolvedFullName = normalizeOptionalText(fullName) || normalizeOptionalText(user.user_metadata?.full_name) || normalizeOptionalText(user.user_metadata?.name);
  ...
  if (member) {
    ...
    if (Object.keys(updates).length) {
      const { data, error } = await supabase.from("team_members").update(updates).eq("id", member.id).select("id, username, full_name, email, phone, auth_user_id, is_active, is_superuser, notes, created_at, last_login_at").single();
  ...
  }).select("id, username, full_name, email, phone, auth_user_id, is_active, is_superuser, notes, created_at, last_login_at").single();
...
export async function getMemberRecordById(memberId) {
  ...
  const supabase = createSupabaseAdminClient();
  if (!supabase) return null;
  const { data, error } = await supabase.from("team_members").select("id, username, full_name, email, phone, auth_user_id, is_active, is_superuser, notes, created_at, last_login_at").eq("id", memberId).maybeSingle();
...
export async function requireMemberSession() {
  ...
  if (!currentMember) return { currentMember: null, session: null, member: null, user: null, error: NextResponse.json({ success: false, message: "Please sign in to continue." }, { status: 401 }) };
  return { currentMember, session: currentMember.session, member: currentMember.member, user: currentMember.user, error: null };
}
