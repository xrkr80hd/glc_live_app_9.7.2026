...
const ORDER_REQUEST_REVIEWER_KEYS = new Set(["pastor", "superuser"]);
  ...
  return Array.from(
    new Set(
      (Array.isArray(roleKeys) ? roleKeys : [])
        .map((key) => normalizeRoleKeyForPolicy(key))
        .filter(Boolean),
    ),
  );
}
...
export function hasAnyRole(roleKeys, requiredRoleKeys) {
  const normalizedCurrent = new Set(
    normalizeRoleKeysForPolicy(roleKeys),
  );
...
export function isOrderRequestSubmitterRole(roleKey) {
  const normalized = normalizeRoleKeyForPolicy(roleKey);
  return ORDER_REQUEST_SUBMITTER_KEYS.has(normalized);
}
